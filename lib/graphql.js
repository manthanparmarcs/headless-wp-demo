...
export async function fetchGraphQL(query, variables = {}, options = {}) {
  ...
  if (options.auth) {
    ...
    if (!authHeaders.Authorization) {
      throw new Error(
        "Missing WordPress credentials for mutations. In Vercel, open Project Settings -> Environment Variables and set WORDPRESS_GRAPHQL_USERNAME + WORDPRESS_GRAPHQL_APPLICATION_PASSWORD, or WORDPRESS_GRAPHQL_AUTH_TOKEN. For local development, set them in .env.local and restart `npm run dev`.",
      );
    }
